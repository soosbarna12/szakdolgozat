...
export function ForecastTile({ data, isLoading }: Readonly<ForecastTileProps>) {
  ...
  function renderSkeletons() {
    ...
    return (
      ...
      >
        <Skeleton variant="rectangular" animation="wave" width="150px" height="120px" sx={{ mb: 2 }} />
      </StyledItem>
    )
  };
    ...
    >
      ...
      <Box sx={{ display: "flex", alignItems: "center", mt: 1 }} color={theme.palette.primary.dark}>
        <Typography data-testid="temperature" variant="h2" sx={{ fontSize: "30px", ml: 1, m: 0 }}>
          {temperatureScaleChanger(TemperatureScale.Celsius, temperatureScale as TemperatureScale, Number(temperature != null ? temperature.toFixed(2) : "N/A"))}
